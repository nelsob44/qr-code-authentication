                ...
                <div class="row container">
                    <form method="POST" action="{{ route('pay') }}" accept-charset="UTF-8" class="form-horizontal" role="form">
                        <div class="row" style="margin-bottom:40px;">
                          ...
                          <p><h3>{{ $qrcode->product_name }} <br><small>£{{ $qrcode->amount }}</small></h3></p>

                            <input type="hidden" name="email" value="nelsob44@gmail.com">
                            <input type="hidden" name="orderID" value="{{ $transaction->id }}">
                            <input type="hidden" name="amount" value="{{ $qrcode->amount * 100}}"> {{--{{ $qrcode->amount }}--}}
                            <input type="hidden" name="quantity" value="1">

                            <input type="hidden" name="metadata" value="{{ json_encode($array = ['buyer_user_id' => $user->id,
                            'buyer_user_email' => $user->email, 'qrcode_id'=> $qrcode->id, 'transaction_id'=> $transaction->id]) }}" > {{-- For other necessary things you want to add to your payload. it is optional though --}}
